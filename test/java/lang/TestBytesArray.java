...
public class TestBytesArray {
    ...
    private static void testValueOf() {
        ...
        check(java.lang.types.BytesArray.valueOf(0L) == java.lang.types.BytesArray.ZERO, 
              "valueOf(0) should return ZERO");
        check(java.lang.types.BytesArray.valueOf(1L) == java.lang.types.BytesArray.ONE, 
              "valueOf(1) should return ONE");
        check(java.lang.types.BytesArray.valueOf(2L) == java.lang.types.BytesArray.TWO, 
              "valueOf(2) should return TWO");

        // check a larger value
        long bigValue = 0x1FFFF_FFFFL; // 8589934591 decimal
        int[] result = java.lang.types.BytesArray.valueOf(bigValue);
    ...
    private static void testCompare() {
        int[] zero = java.lang.types.BytesArray.ZERO;
        int[] one  = java.lang.types.BytesArray.ONE;
        int[] two  = java.lang.types.BytesArray.TWO;

        // zero < one
        check(java.lang.types.BytesArray.compare(zero, one) < 0, "zero < one");
        // two > one
        check(java.lang.types.BytesArray.compare(two, one) > 0, "two > one");
        // zero == zero
        check(java.lang.types.BytesArray.compare(zero, zero) == 0, "zero == zero");

        int[] x = new int[]{0x1234, 0xABCD};
        int[] y = new int[]{0x1234};
        check(java.lang.types.BytesArray.compare(x, y) > 0, "longer array with same top word is bigger");

        int[] xCopy = new int[]{0x1234, 0xABCD};
        check(java.lang.types.BytesArray.compare(x, xCopy) == 0, "identical arrays => compare == 0");
    }

    private static void testStripLeadingZeroes() {
        // no leading zeros => same array
        int[] a = new int[]{0x1234, 0xABCD};
        int[] sA = java.lang.types.BytesArray.stripLeadingZeroes(a);
        check(sA == a, "no leading zero => same array returned");

        // leading zeros
        int[] b = new int[]{0, 0, 0xFEED, 0xBEEF};
        int[] sB = java.lang.types.BytesArray.stripLeadingZeroes(b);
    ...
    private static void testBitwiseOps() {
        int[] one = java.lang.types.BytesArray.ONE;   // [0x00000001]
        int[] two = java.lang.types.BytesArray.TWO;   // [0x00000002]

        // 1 & 2 => 0
        int[] andRes = java.lang.types.BytesArray.and(one, two);
        check(andRes.length == 0, "1 & 2 => zero-length array => 0");

        // 1 | 2 => 3
        int[] orRes = java.lang.types.BytesArray.or(one, two);
        check(orRes.length == 1 && orRes[0] == 3, "1 | 2 => [3]");

        // 1 ^ 2 => 3
        int[] xorRes = java.lang.types.BytesArray.xor(one, two);
    ...
    private static void testSetClearFlipBit() {
        ...
        // setBit(zero, 0) => [1]
        int[] set = java.lang.types.BytesArray.setBit(zero, 0);
        check(set.length == 1 && set[0] == 1, "setBit(zero,0) => [1]");

        // clearBit(one,0) => zero
        int[] cleared = java.lang.types.BytesArray.clearBit(one, 0);
        check(cleared.length == 0, "clear bit0 on [1] => zero array");
    
        // flipBit(one,1) => [3]
        int[] flipped = java.lang.types.BytesArray.flipBit(one, 1);
    ...
    private static void testArithmeticSub() {
        ...
        // 2 - 1 => 1
        int[] diff = java.lang.types.BytesArray.sub(two, one);
        check(diff.length == 1 && diff[0] == 1, "2-1 => 1 => [1]");

        // 0x12345678 - 0x5678 => 0x12340000
        int[] left  = new int[]{0x1234, 0x5678};
        int[] right = new int[]{0x5678};
        int[] out   = java.lang.types.BytesArray.sub(left, right);
    ...
    private static void testMulMod() {
        ...
        int[] product = java.lang.types.BytesArray.mulmod(two, three, five);
    ...
    private static void testFromBigInteger() {
        ...
        // maxWidth = 4
        int[] result = java.lang.types.BytesArray.from(bigVal, 4); 
    ...
    private static void testRandomArithmetic() {
        ...
        for(int i = 0; i < 5; i++) {
            ...
            int[] arrX = java.lang.types.BytesArray.valueOf(x);
            int[] arrY = java.lang.types.BytesArray.valueOf(y);

            // naive check for sum
            long sumL = x + y; // may overflow 64 bits, but okay for quick test
            int[] arrSum = java.lang.types.BytesArray.add(arrX, arrY, -1);
            ...
            // no crash => test pass
            int[] arrDiff = java.lang.types.BytesArray.sub(arrX, arrY);
        }
    }
